...
이 문서는 SwiftProtobuf 플러그인에 의해 생성 된 코드를 사용하는 방법을 설명합니다.

생성된 코드는 SwiftProtobuf 라이브러리에 의존하며. 
실제 속성 자체를 제외하고, 설명 된 아래의 대부분의 함수는 생성 된 코드에서 명시 적이지 않습니다.
오히려 대부분은 런타임 라이브러리의 공통 공유 프로토콜에 나타나며,
그것들은 여기에 더 이해하기 쉽게 수집됩니다.

## Message API

입력 프로토 파일의 Message는 플러그인에 의해 생성 된 결과물에서 Swift 구조체(Structs)를 생성합니다.
이 구조체는`SwiftProtobuf.Message`를 따르며 
모든 객체(필드, 메시지에 대한 기본 정보, 표준 이니셜 라이저 및 직렬화 및 직렬화 복원 방법)에 대해
Swift 프로퍼티(properties)를 제공합니다.

다음은 아래 예제의 기초가 되는 간단한 proto3 입력 파일입니다
...
위의 코드에서 생성 된 구조체의 API는 다음과 같습니다.
(앞서 언급했듯이, 플러그인에서 생성 된 Swift 코드를 열어 볼 때 나타나는 것은 아닙니다.
여기에는 라이브러리에있는 확장 기능의 메소드가 많이 포함되어 있으며 라이브러리에서 
내부적으로 사용하려는 생성 된 코드의 많은 세부 사항은 생략됩니다.)

```swift
public struct Example: SwiftProtobuf.Message {
  // 생성 된 struct는 상수 프로퍼티를 반영합니다.
  // 메시지에 대한 기본 정보 :
  ...
  // proto의 각 필드에 대해 public 프로퍼티가 만들어집니다.
...
  // 메시지는 protobuf 바이너리 형식을 사용하여 Data 객체로 직렬화 또는 비 직렬화 될 수 있습니다. 
  // Data 객체로 직렬화 또는 비 직렬화 될 수 있습니다.
  //`partial`을`true`로 설정하면 필수 필드의 체크가 억제됩니다.
  // 확장을 사용하는 중첩 된 proto2 형식 메시지를
  // 디코딩 할 때 확장 맵이 필요할 수 있습니다.
  // 자세한 내용은 아래를 참조하십시오.
  func serializedData() throws -> Data
  init(serializedData: Data) throws {
  init(serializedData: Data, extensions: ExtensionMap? = nil, partial: Bool = false) throws

  // UTF8로 인코딩 된 Data 객체 또는 String으로 JSON 형식으로 메시지를 
  // 직렬화 또는 비 직렬화 할 수 있습니다.
  func jsonUTF8Data() throws -> Data
  init(jsonUTF8Data: Data) throws
  func jsonString() throws -> String
  init(jsonString: String) throws

  // 메시지를 직렬화하거나 Protobuf TextFormat로 직렬화 해제 할 수 있습니다.
  func textFormatString() -> String
  init(textFormatString: String) throws

  // 이들은 직렬화 및 직렬화 해제 메커니즘에 의해
  // 내부적으로 사용되는 생성 된 메소드입니다.
  // 일반적으로 직접 호출하지 않아야합니다.
